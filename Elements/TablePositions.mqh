...
class TablePositions : public Table
{
   ...
   private:     
      ...
      {
         //Из таблицы исторических позиций ничего не может быть удалено.
         ...
         for(int i = 0; i < total; i++)
         {
            ...
            //Если требуется также удалить все отображаемые сделки
            if(tbox.State() == BOX_TREE_RESTORE)
               tbox.OnPush();
            workArea.Delete(i);
         }
      ...
      void AddPosition(EventCreatePos* event)
      {
         ...
         if(!IsItForMe(pos))return;
         //Возможно позиция, которую необходимо обновить уже есть в списке
         int total = workArea.ChildsTotal();
         int i = 0;
         for(; i < total; i++)
         {
         ...
         //Если позиция так и не была найдена, создаем ее.
         if(i < total-1)return;
         PosLine* nline = new PosLine(workArea, TableType(), pos);
         workArea.Add(nline);
