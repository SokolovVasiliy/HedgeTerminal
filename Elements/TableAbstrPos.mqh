...
class AbstractPos : public Line
{
   public:
      ...
      /// \param cType - идентификатор ячейки, которую необходимо добавать.
      ///
      bool AddColumn(ENUM_COLUMN_TYPE cType)
      {
         return false;
      }
      ///
      /// Обновляет значение ячейки с идентификатором cType
      /// \param cType - идентификатор ячейки, значение которой надо обновить.
      ///
      bool RefreshColumn(ENUM_COLUMN_TYPE cType)
      {
         return false;
      }
      ///
